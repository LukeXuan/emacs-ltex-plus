...
| `lsp-ltex-plus-sentence-cache-size` | R | Size of the LanguageTool `ResultCache` in sentences. The default and recommended value `0` disables the local LanguageTool server's own cache entirely: ltex-ls-plus keeps its own per-paragraph fragment cache (see `lsp-ltex-plus-max-fragment-size`), which supersedes LanguageTool's caching. Use a positive value to turn it back on, but be aware that for the edit loop this is redundant and only adds CPU and memory overhead with no additional benefit. *Type:* integer; *default:* `0`. | X | X |
